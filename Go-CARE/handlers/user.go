...
func AllocateResource(c *gin.Context) {
	...
	// Fetch the available resources from the database based on the requested resource type
	var resource model.Resources
	if err := database.DB.Where("type = ? AND availability = ?", request.ResourceType, "true").First(&resource).Error; err != nil {
	...
	successMessage := fmt.Sprintf("Successfully allocated %d %s", quantity, request.ResourceType)
